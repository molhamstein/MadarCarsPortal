...
export class brandsComponent implements OnInit {
  ...
  addUser() {
    this.mainServ.globalServ.goTo("add-brand")
  }

  deactivate(id) {
    var mainThis = this;
    this.translateService.get('MESSAGES.DEACTIVELOCATION').subscribe((res: string) => {
      this.dialogServ.confirmationMessage(res, "brands/deactivate/" + id, {}, false, function () {
        mainThis.inisilaize()
      }, "delete")
    })
  }
  ...
  activate(id, name) {
    ...
    this.translateService.get('MESSAGES.ACTIVELOCATION').subscribe((res: string) => {
      this.dialogServ.confirmationMessage(res, "brands/activate/" + id, {}, false, function () {
        mainThis.inisilaize()
      }, "put")
    })
  }
